...
```{r}
unique(data10194['target_taxon_name'])
```
...
Only 1000 records are given when trying to extract data.   
```{r}
...
Functions: Fields are not shown, could only show default fields? c("source_taxon_external_id","source_taxon_name", "source_taxon_path", "source_specimen_life_stage", "interaction_type", "target_taxon_external_id", "target_taxon_name", "target_taxon_path", "target_specimen_life_stage", "latitude", "longitude", "study_citation", "study_external_id", "study_source_citation")
```{r}
get_interactions_by_taxa('Larus marinus', interactiontype = 'eats', showfield = c('target_specimen_frequency_of_occurrence',	
